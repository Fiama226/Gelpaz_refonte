# GELPAZ IMMO — Comprehensive UX/UI Audit & Design Recommendations

> **Statut de mise en œuvre (itération 2).** **P0 (§1–8)** : header/logo, jeu d’icônes SVG unique, plancher typographique, contrastes WCAG, correctif de chevauchement mobile, formulaire de contact fonctionnel (honeypot, états succès/erreur), WhatsApp (bouton flottant + CTA pré-remplis + bloc footer), favicons/Open Graph/canonical/JSON-LD, vraie page 404, URLs propres, `screenshots.zip` retiré de la racine publique, `.htaccess` (cache, compression, en-têtes de sécurité), `/mois` sur le loyer, lien d’évitement clavier.
>
> **P1 déjà traité** : images servies en variantes **835 px** (au lieu des vignettes 525 px) avec `srcset`, repli automatique et `width/height` (anti-CLS) ; **galeries par bien** avec de vraies photos du média Gelpaz (fini les visuels sans rapport) + visionneuse clavier/Échap ; **articles réellement distincts** (slug, date ISO, contenu, liens de partage) au lieu de six cartes pointant vers la même page ; **mentions légales + politique de confidentialité** avec liens dans le footer et sous la case de consentement ; **carte OpenStreetMap** sans clé d’API à la place des visuels CSS ; **`robots.txt` + sitemap généré** ; bloc WhatsApp dans le pied de page ; encart CTA dans la colonne article.
>
> **Reste à planifier (P1/P2)** : photos/noms réels de l’équipe, logos de partenaires authentiques, auto-hébergement + conversion WebP/AVIF des visuels, réorganisation complète de `style.css` (échelle d’espacement, reset en tête, minification/versionnage), lightbox étendue aux articles, mesure d’audience. Voir `README.md` (section « Reste à faire »). Aperçu visuel statique : `docs/preview/ui-preview.html`.
