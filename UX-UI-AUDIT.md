...
> **Statut de mise en œuvre (itération 1).** Les actions **P0 (§1–8)** sont implémentées : header/logo, jeu d’icônes SVG unique, plancher typographique, contrastes WCAG, correctif de chevauchement mobile, formulaire de contact fonctionnel (honeypot, états succès/erreur), WhatsApp (bouton flottant + CTA pré-remplis + bloc footer), favicons/Open Graph/canonical/JSON-LD, vraie page 404, URLs propres, `screenshots.zip` retiré de la racine publique, `.htaccess` (cache, compression, en-têtes de sécurité). Également traité : `page_url()` en URLs propres, `/mois` sur le loyer, lien d’évitement clavier. Voir le détail dans `README.md`. Aperçu visuel statique : `docs/preview/ui-preview.html`. Le reste des items **P1/P2** (images haute résolution auto-hébergées, équipe/partenaires réels, articles distincts, carte interactive, mentions légales, refonte complète du CSS) reste à planifier.
